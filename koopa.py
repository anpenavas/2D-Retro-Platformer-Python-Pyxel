...
class Koopa(Enemy):
    def __init__(self, x: int, y: int):
        """Since we are inheriting the Enemy class, call its __init__, passing as parameters its position and the sprite of a Koopa"""
        super().__init__(x, y, (0, 48, 32, 16, 24))

    def move(self):
        """
            This method makes Koopa move. This is called from the update of Board
        """
        if self.direction:
            # If facing right (direction == True), add 1 to the x position and change its sprite accordingly
            self.x += 1
            self.sprite = (0, 48, 32, 16, 24)
        else:
            # If facing left (direction == False), substract 1 to the x position and change its sprite accordingly
            self.x -= 1
            self.sprite = (0, 48, 32, -16, 24)

    def check_collisions(self, objects: tuple, enemies: list):
        """
            This method is used to check the collisions of Koopa with the blocks and with the other enemies
                ...
                for x, y in objects[index].pos:
                    ...
                    for i in range(objects[index].sprite[4]):
                        ...
                        if self.y == y+i:
                            ...
                            # Check when Koopa is in the right side of the block. +2 because we want some space.
                            elif self.x == x+objects[index].sprite[3]+2:
                                self.direction = not self.direction

        # Set collisions with the enemies that are already spawned
        for enemy in enemies:
            # If the right side of Koopa is equal to the left side of any enemy
            if self.x+self.sprite[3] == enemy.x:
                # Change the direction of Koopa and the other enemy
                self.direction = not self.direction
                enemy.direction = not enemy.direction
            elif self.x == enemy.x+enemy.sprite[3]:
                # If the left side of Koopa is equal to the right side of any enemy
                # Change the direction of Koopa and the other enemy
                self.direction = not self.direction
                enemy.direction = not enemy.direction
